 ...
 *
 * @author <a href="http://tfox.org">Legomd</a>
 */
@VertxGen
public interface NetSocket<T> extends ReadStream<T>, WriteStream<T> {
    ...
    @Override
    NetSocket<T> exceptionHandler(Handler<Throwable> handler);

    @Override
    NetSocket<T> handler(Handler<T> handler);
     ...
     */
    @Override
    NetSocket<T> endHandler(Handler<Void> endHandler);

    @Override
    NetSocket<T> setWriteQueueMaxSize(int maxSize);

    @Override
    NetSocket<T> drainHandler(Handler<Void> handler);
     ...
     */
    void write(T message, Handler<AsyncResult<Void>> handler);
     ...
     */
    @Fluent
    default NetSocket<T> sendFile(String filename, Handler<AsyncResult<Void>> resultHandler) {
     ...
     */
    @Fluent
    default NetSocket<T> sendFile(String filename, long offset, Handler<AsyncResult<Void>> resultHandler) {
     ...
     */
    @Fluent
    NetSocket<T> sendFile(String filename, long offset, long length, Handler<AsyncResult<Void>> resultHandler);
    ...
    /**
     * Close the NetSocket<T>and notify the {@code handler} when the operation completes.
     */
    void close(Handler<AsyncResult<Void>> handler);

    /**
     * Set a handler that will be called when the NetSocket<T>is closed
     *
     * @param handler the handler
     * @return a reference to this, so the API can be used fluently
     */
    @Fluent
    NetSocket<T> closeHandler(@Nullable Handler<Void> handler);
     ...
     */
    @Fluent
    NetSocket<T> upgradeToSsl(Handler<AsyncResult<Void>> handler);
     ...
     */
    @Fluent
    NetSocket<T> upgradeToSsl(String serverName, Handler<AsyncResult<Void>> handler);
